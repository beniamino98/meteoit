...
xml_find_attr <- function(x, xpath, attrnames = "", attribute = "class", ns = xml_ns(x), exact_match = TRUE){
  ...
  attr_selected <- xml2::xml_attr(all_elements, attribute)
  
  if(exact_match){
    class_index <- which(attr_selected %in% attrnames)
  } else {
    class_index <- which(stringr::str_detect(attr_selected, attrnames[1]))
  }

  return(all_elements[class_index])
}
...
meteo_city_month_year <- function(city = "", month = "", imp.year = "", verbose = TRUE){
  ...
  stazione <- xml2::xml_text(xml_find_attr(html, ".//div", attrnames = "smalllinks")[1])
  stazione <- stringr::str_remove_all(stazione, "Dati registrati dalla stazione meteo di  ")
  ...
  if(verbose) {
    message("Importazione: ", city_name, " (Anno: ", imp.year, ", Mese: ", month, ")")
  }
...
# FUNCTION: MeteoIT 
meteo_it <- function(city = "", start_year = "2021", end_year = "2022", verbose = TRUE, complete_info = FALSE, filename = NULL){
  ...
  if(!is.null(filename)){
    ...
    if (verbose) {
      message("Importazione Parziale Salvata con il nome di: ", filename)
    }
  }
  ...
  if (complete_info) {
    ...
    if (verbose) {
      message("Importazione con informazioni dettagliate in corso...")
    }
    
    # aggiunta informazioni complete 
    df_completo <- add_complete_info(df_importazione, verbose = verbose)
    df_completo <- meteo_it_cleaner(df_completo)
    ...
    if (!is.null(filename)) {
      ...
      if (verbose) {
        message("Importazione Completa Salvata con il nome di: ", filename)
      }
    }
    return(df_completo)
  }
...
add_complete_info <- function(df, verbose = TRUE){
  ...
  if(verbose) {
    message("Tempo Stimato: ", round(n_iter), " Secondi")
  }
  ...
  all_df_info <- list()
  safe_daily_info <- purrr::safely(daily_info)
  for(i in 1:n_iter) {
    
    result <- safe_daily_info(df[i,])$result
  ...
  end_time <- Sys.time()
  if (verbose) {
    message("Tempo Impiegato: ", round(end_time-start_time, 3), " Minuti...")
  } 
...
# FUNCTION: meteo_it_cleaner 
# pulizia standard dei dati importati 
meteo_it_cleaner <- function(df){
  ...
  df <- suppressWarnings(dplyr::mutate(df, 
    ...
    # se NA no pioggia quindi 0
    rain = ifelse(is.na(rain), 0, rain),
    
    # riduco etichette ridondanti
    phenomena = ifelse(phenomena == "Pioggia - Temporale Pioggia - Temporale - Nebbia", "Pioggia - Temporale - Nebbia", phenomena),
    
    # ricalcolo la temperatura media come (massimo+minimo)/2
    mean_temp = (max_temp + min_temp)/2
    
  )) 
  
  df <- dplyr::select(df, -url_info, -precip)
